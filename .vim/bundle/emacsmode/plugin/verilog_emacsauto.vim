...
if !hasmapto('<Plug>VerilogEmacsAutoAdd')
   ...
   map <unique> <Leader>y <Plug>VerilogEmacsAutoAdd
endif
...
function s:Add()
   ...
   w! %.emacsautotmp
   !emacs -batch -l ~/.vim/bundle/emacsmode/verilog-mode.el %.emacsautotmp -f verilog-batch-auto
   %!cat %.emacsautotmp 
   if &expandtab
      retab
...
function s:Delete()
   ...
   " maybe add /tmp to the temporary filename
   w! %.emacsautotmp
   !emacs -batch -l ~/.vim/bundle/emacsmode/verilog-mode.el %.emacsautotmp -f verilog-batch-delete-auto
   %!cat %.emacsautotmp 
   !rm %.emacsautotmp
endfunction
