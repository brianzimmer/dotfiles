...
if !hasmapto('<Plug>VerilogEmacsAutoFormat')
   map <unique> <Leader>f <Plug>VerilogEmacsAutoFormat
   "map <unique> <C-A> <Plug>VerilogEmacsAutoAdd
endif
...
noremap <unique> <script> <Plug>VerilogEmacsAutoFormat <SID>Format
noremap <SID>Add    :call <SID>Add()<CR>
noremap <SID>Delete :call <SID>Delete()<CR>
noremap <SID>Format :call <SID>Format()<CR>
...
function s:Format()
   ...
   " maybe add /tmp to the temporary filename
   w! %.emacsautotmp
   !emacs -batch -l ~/.vim/verilog-mode.el %.emacsautotmp -f verilog-batch-indent
   %!cat %.emacsautotmp 
   if &expandtab
      retab
      let &tabstop=s:save_tabstop
